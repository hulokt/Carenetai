...
const Onboarding = () => {
  const { createUser, currentUser } = useStateContext();
  ...
  // Redirect to home if user already exists (completed onboarding)
  useEffect(() => {
    if (currentUser) {
  ...
  const handleOnboarding = async (e) => {
    ...
    if (newUser) {
      // Redirect to dashboard (home page) after successful onboarding
      navigate("/", { replace: true });
    }
  };
    ...
    <div className="flex min-h-screen items-center justify-center bg-[#13131a]">
      <div className="w-full max-w-md rounded-xl bg-[#1c1c24] p-8 shadow-lg">
        <div className="mb-6 text-center">
          ...
          <h1 className="mb-2 text-4xl font-bold font-jakarta" style={{ color: '#1F4E89' }}>
            Carenetai
          </h1>
          <p className="text-gray-400 text-sm italic mb-4">A community for cancer</p>
          <h2 className="text-5xl font-bold text-white mb-2">👋</h2>
